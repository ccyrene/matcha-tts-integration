...
from converter.utils.model import sequence_mask
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        a,b,c = shape
        x = x * self.gamma.view(a, b, c) + self.beta.view(a, b, c)
        return x
...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        * `x` is the Tensor at the head of a key or a query with shape `[seq_len, batch_size, n_heads, d]`
        ...
        x_cat = torch.cat((x_rope, x_pass), dim=-1)

        # return rearrange(torch.cat((x_rope, x_pass), dim=-1), "t b h d -> b h t d")
        return x_cat.permute(1, 2, 0, 3).contiguous()
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, c, attn_mask:Optional[torch.Tensor]=None):
        ...
        x, self.attn = self.attention(query=q, key=k, value=v, mask=attn_mask)
...
class TextEncoder(nn.Module):
    ...
    # def sequence_mask(self, length, max_length:Optional[int]=None):
    #     if max_length is None:
    #         max_length = length.max()
    #     x = torch.arange(max_length, dtype=length.dtype, device=length.device)
    #     return x.unsqueeze(0) < length.unsqueeze(1)

    def forward(self, x, x_lengths, spks:Optional[torch.Tensor]=None):
